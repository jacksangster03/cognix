...
import type { ProvenanceFlags } from "@/lib/ai/brief-schema"
...
function ScoreRing({ score }: { score: number }) {
  ...
  return (
    <svg width="120" height="120" className="-rotate-90">
      <circle cx="60" cy="60" r={r} fill="none" stroke="currentColor" strokeWidth="6" className="text-zinc-800" />
      <circle
        cx="60" cy="60" r={r} fill="none" strokeWidth="6" strokeLinecap="round"
...
export function ReadinessHero({ scores, mode, headline, provenance }: ReadinessHeroProps) {
  const label = provenance ? getDataStateLabel(provenance) : null
  const showAmber = label && label !== "Live personal data" && label !== "Limited data"
  const showDim = label === "Limited data"
